...
function aihl_render_manifest_json_page(): void {
	...
	<div class="wrap aihl-mj-wrap">
		...
		<section class="aihl-mj-card aihl-mj-editor-card">
			...
			<textarea id="aihl-manifest-json" class="aihl-mj-editor" readonly spellcheck="false"><?php echo esc_textarea($json); ?></textarea>
		</section>
	...
	<style>
	.aihl-mj-wrap{margin:0}.aihl-mj-intro,.aihl-mj-card>header,.aihl-mj-toolbar,.aihl-mj-snapshot-form,.aihl-mj-history article{display:flex;align-items:center;justify-content:space-between;gap:16px}.aihl-mj-intro{margin-bottom:16px}.aihl-mj-intro h2,.aihl-mj-card h2{margin:0 0 4px}.aihl-mj-intro p,.aihl-mj-card header p{margin:0;color:#646970}.aihl-mj-card{margin:0 0 20px;padding:18px;background:#fff;border:1px solid #dcdcde;border-radius:4px}.aihl-mj-editor-card{padding:0;overflow:hidden}.aihl-mj-toolbar{padding:10px 14px;background:#f6f7f7;border-bottom:1px solid #dcdcde}.aihl-mj-toolbar span{margin-left:auto;color:#646970}.aihl-mj-editor{display:block;width:100%;min-height:68vh;padding:18px;resize:vertical;border:0;border-radius:0;background:#111827;color:#dbeafe;font:14px/1.55 Consolas,Monaco,monospace;box-sizing:border-box}.aihl-mj-editor:focus{outline:2px solid #2271b1;outline-offset:-2px}.aihl-mj-snapshot-form{justify-content:flex-start;margin-top:16px}.aihl-mj-snapshot-form label{display:grid;flex:1 1 420px;max-width:640px;gap:5px}.aihl-mj-snapshot-form label span{font-weight:600}.aihl-mj-snapshot-form input{width:100%}.aihl-mj-count{display:grid;min-width:30px;height:30px;place-items:center;background:#eef6fc;color:#135e96;border-radius:4px;font-weight:700}.aihl-mj-history{margin-top:14px;border-top:1px solid #dcdcde}.aihl-mj-history article{justify-content:flex-start;padding:12px 0;border-bottom:1px solid #dcdcde}.aihl-mj-history article>div{display:grid;flex:1;gap:3px}.aihl-mj-history small,.aihl-mj-empty{color:#646970}@media(max-width:782px){.aihl-mj-intro,.aihl-mj-card>header,.aihl-mj-snapshot-form,.aihl-mj-history article{align-items:stretch;flex-direction:column}.aihl-mj-snapshot-form label{flex-basis:auto}.aihl-mj-editor{min-height:60vh}}
	</style>
	<script>(function(){var editor=document.getElementById('aihl-manifest-json'),copy=document.getElementById('aihl-mj-copy'),status=document.getElementById('aihl-mj-status');if(!editor||!copy)return;copy.addEventListener('click',function(){navigator.clipboard.writeText(editor.value).then(function(){status.textContent='<?php echo esc_js(__('Copiato', AIHL_TEXT_DOMAIN)); ?>';setTimeout(function(){status.textContent='<?php echo esc_js(__('Sola lettura', AIHL_TEXT_DOMAIN)); ?>';},1600);});});})();</script>
	<?php
}
